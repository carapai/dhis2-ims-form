...
export const ItemField: FC<ItemFieldProps> = observer(({ item, onBlur, otherClasses = '' }) => {
  ...
  switch (item.valueType) {
    ...
    case 'INTEGER':
      return <Form.Item key={item.id} className={`m-0 mt-2 mb-2 p-1 ${otherClasses}`} labelAlign="right" name={item.id} label={String(item.displayFormName)}>
        <InputNumber min={0} disabled={disabled} onBlur={onBlur(item.id)} className={item.className} />
      </Form.Item>
    case 'TRUE_ONLY':
    case 'BOOLEAN':
      return <Form.Item key={item.id} className={`m-0 mt-2 mb-2 p-1 ${otherClasses}`} valuePropName="checked" labelAlign="right" name={item.id} label={String(item.displayFormName)}>
